...
public class DeploymentManagerImpl extends AbstractProgressManager implements DeploymentManager {
        ...
        for (final MavenArtifact mavenArtifact : artifacts) {
            ...
            for (final MavenModule mavenModule : deploymentDependencies) {
                ...
                if (dependencyFailedToDeploy) {
                    msg = "Cannot deploy artifact because the dependent module " + mavenModule.getName()
                            + " failed to deploy before.";
            ...
            if (!dependencyFailedToDeploy) {
                deployedDestinations =
                        deployArtifactToDestinations(releaseArtifact, execution, environment, activeRelease,
                                failedToDeployMavenModules);
            }
    ...
    /**
     * Calculates the amount to use per artifact for keeping track of the
     ...
     * @param execution
     *            The release execution that is need to keep track of the
     *            release.
     * @param environment
     ...
     *            Collection containing the {@link MavenModule} objects that
     *            failed to deploy.
     * @return Returns a {@link List} containing the {@link Destination} objects
     *         where the artifact was deployed to.
     */
    private List<Destination> deployArtifactToDestinations(final MavenArtifact releaseArtifact, final ReleaseExecution execution,
        ...
        if (destinations.isEmpty()) {
            releaseHistoryManager.registerActivity(execution, "Retrieved " + numberOfAvailableDestinations
        ...
        for (final Destination destination : destinations) {
            ...
            /*
             * Check if we are supposed to deploy to the specific environment or
             * not.
             */
    ...
    /**
     * Initializes the specified release so all information can be shown by the
     * calling code.
     * 
     * @param activeRelease
     ...
     *            The parent module where some data will be retrieved from to
     *            identify the snapshot artifact.
     * @param releaseArtifact
     *            The artifact that is going to be released.
     * @return Returns the {@link MavenArtifactSnapshot}.
     */
    private MavenArtifactSnapshot retrieveCurrentSnapshot(final ReleaseExecution execution,
            final MavenModule releaseArtifactParentModule,
