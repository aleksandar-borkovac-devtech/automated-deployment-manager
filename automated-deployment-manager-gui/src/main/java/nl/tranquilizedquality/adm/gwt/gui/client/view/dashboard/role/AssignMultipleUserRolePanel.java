...
public class AssignMultipleUserRolePanel extends AbstractGridPanel {
    ...
    protected void initializeWidgets() {
        ...
        /*
         * Add listener so the table will refresh after selecting a different
         * scope.
         */
        final SelectionChangedListener<ModelData> selectionChangedListener = new SelectionChangedListener<ModelData>() {

            @Override
            public void selectionChanged(final SelectionChangedEvent<ModelData> se) {
                final ModelData selectedItem = se.getSelectedItem();

                if (selectedItem != null) {
                    final BeanModel model = (BeanModel) selectedItem;
                    final ClientScope scope = model.getBean();
                    sc.setScope(scope);
                }
        ...
        scopes.addSelectionChangedListener(selectionChangedListener);

        formPanel.add(scopes);

        final LayoutContainer container = new LayoutContainer(new FitLayout());
