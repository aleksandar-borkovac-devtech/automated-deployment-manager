...
public class AdmNavigationPanel extends AbstractNavigationPanel<NavigationConstants, AdmIcons> {
	...
	private void addReleaseManagementItem(final List<TreeMenuItem> items) {
		...
		if (findReleaseMenuItem != null || addNewReleaseMenuItem != null) {
			final TreeMenuItem userManagementItem = new TreeMenuItem("Release Management", AdmTabs.RELEASE_MANAGEMENT_TAB, new BaseTreeModel[] { findReleaseMenuItem });
	...
	private void addArtifactManagementItem(final List<TreeMenuItem> items) {
		final TreeMenuItem findMavenModuleMenuItem = createMenuItem("SEARCH_ARTIFACT", "Find artifacts", AdmTabs.ARTIFACT_MANAGEMENT_TAB);

		if (findMavenModuleMenuItem != null) {
			final TreeMenuItem userManagementItem = new TreeMenuItem("Artifact Management", AdmTabs.ARTIFACT_MANAGEMENT_TAB, new BaseTreeModel[] { findMavenModuleMenuItem });
			items.add(userManagementItem);
		}
	}
	...
	private void addMavenModuleManagementItem(final List<TreeMenuItem> items) {
		final TreeMenuItem findMavenModuleMenuItem = createMenuItem("SEARCH_MAVEN_MODULE", "Find Maven Modules", AdmTabs.MAVEN_MODULE_MANAGEMENT_TAB);
		final TreeMenuItem addNewMavenModuleMenuItem = createMenuItem("ADD_MAVEN_MODULE", "Add new module", AdmTabs.MAVEN_MODULE_DETAILS_TAB);

		if (findMavenModuleMenuItem != null || addNewMavenModuleMenuItem != null) {
			final TreeMenuItem userManagementItem = new TreeMenuItem("Maven Module Management", AdmTabs.MAVEN_MODULE_MANAGEMENT_TAB, new BaseTreeModel[] { findMavenModuleMenuItem,
	...
	private void addDestinationManagementItem(final List<TreeMenuItem> items) {
		final TreeMenuItem findDestinationMenuItem = createMenuItem("SEARCH_DESTINATION", "Find destinations", AdmTabs.DESTINATION_MANAGEMENT_TAB);
		final TreeMenuItem addNewDestinationMenuItem = createMenuItem("ADD_DESTINATION", "Add new destination", AdmTabs.DESTINATION_DETAILS_TAB);

		if (findDestinationMenuItem != null || addNewDestinationMenuItem != null) {
			final TreeMenuItem userManagementItem = new TreeMenuItem("Destination Management", AdmTabs.DESTINATION_MANAGEMENT_TAB, new BaseTreeModel[] { findDestinationMenuItem,
	...
	private void addEnvironmentManagementItem(final List<TreeMenuItem> items) {
		final TreeMenuItem findEnvironmentMenuItem = createMenuItem("SEARCH_ENVIRONMENT", "Find environments", AdmTabs.ENVIRONMENT_MANAGEMENT_TAB);
		final TreeMenuItem addNewEnvironmentMenuItem = createMenuItem("ADD_ENVIRONMENT", "Add new environment", AdmTabs.ENVIRONMENT_DETAILS_TAB);

		if (findEnvironmentMenuItem != null || addNewEnvironmentMenuItem != null) {
			final TreeMenuItem userManagementItem = new TreeMenuItem("Environment Management", AdmTabs.ENVIRONMENT_MANAGEMENT_TAB, new BaseTreeModel[] { findEnvironmentMenuItem,
	...
	private void addHostManagementItem(final List<TreeMenuItem> items) {
		...
		if (findHostMenuItem != null || addNewHostMenuItem != null) {
			final TreeMenuItem userManagementItem = new TreeMenuItem("Host Management", AdmTabs.DESTINATION_HOST_MANAGEMENT_TAB, new BaseTreeModel[] { findHostMenuItem, addNewHostMenuItem });
			items.add(userManagementItem);
		}
	}
	...
	private void addRepositoryManagementItem(final List<TreeMenuItem> items) {
		final TreeMenuItem findRepositoryMenuItem = createMenuItem("SEARCH_REPOSITORY", "Find repositories", AdmTabs.REPOSITORY_MANAGEMENT_TAB);
		final TreeMenuItem addNewRepositoryMenuItem = createMenuItem("ADD_REPOSITORY", "Add new repository", AdmTabs.REPOSITORY_DETAILS_TAB);

		if (findRepositoryMenuItem != null || addNewRepositoryMenuItem != null) {
			final TreeMenuItem userManagementItem = new TreeMenuItem("Repository Management", AdmTabs.REPOSITORY_MANAGEMENT_TAB, new BaseTreeModel[] { findRepositoryMenuItem, addNewRepositoryMenuItem });
			items.add(userManagementItem);
		}
	}
	...
	private void addUserManagementItem(final List<TreeMenuItem> items) {
		...
		if (findUsersMenuItem != null || addNewUserMenuItem != null) {
			final TreeMenuItem userManagementItem = new TreeMenuItem("User Management", AdmTabs.USER_MANAGEMENT_TAB, new BaseTreeModel[] { findUsersMenuItem, addNewUserMenuItem });
			items.add(userManagementItem);
		}
	...
	private void addUserGroupItem(final List<TreeMenuItem> items) {
		final TreeMenuItem findUserGroupsMenuItem = createMenuItem("SEARCH_USER_GROUP", "Find user groups", AdmTabs.USER_GROUP_MANAGEMENT_TAB);
		final TreeMenuItem addNewUserGroupMenuItem = createMenuItem("ADD_USER_GROUP", "Add new user group", AdmTabs.USER_GROUP_DETAILS_TAB);

		if (findUserGroupsMenuItem != null || addNewUserGroupMenuItem != null) {
			final TreeMenuItem userManagementItem = new TreeMenuItem("User Group Management", AdmTabs.USER_GROUP_MANAGEMENT_TAB, new BaseTreeModel[] { findUserGroupsMenuItem, addNewUserGroupMenuItem });
			items.add(userManagementItem);
		}
	}
