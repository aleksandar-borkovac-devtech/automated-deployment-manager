...
public class UserManagementTable extends AbstractGridPanel {
	...
	protected List<ColumnConfig> createColumns() {
		...
		final GridCellRenderer<BeanModel> renderer = new GridCellRenderer<BeanModel>() {
			...
			@Override
			public Object render(final BeanModel model, final String property, final ColumnData config, final int rowIndex, final int colIndex, final ListStore<BeanModel> store,
					final Grid<BeanModel> grid) {

				final ClientUser user = model.getBean();

				return "<img src=\"" + GWT.getHostPageBaseURL() + "services/images/user/" + user.getId() + "?" + new Date().getTime() + "\" width=\"67px\" height=\"90px\" />";
			}
		};
